...
async def skip_thumbnail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    session["thumbnail"] = "default_thumbnail.jpg"  # You can specify your default thumbnail here.
    session["waiting_for_thumbnail"] = False
...
def split_video(input_path, tempdir, max_mb):
    split_cmd = [
        "ffmpeg", "-i", input_path, "-c", "copy", "-f", "segment", "-segment_time", "600",  # 10-min chunks
...
def main():
    ...
    app.add_handler(MessageHandler(filters.PHOTO | filters.Document.IMAGE, handle_thumbnail))
    app.add_handler(CallbackQueryHandler(handle_merge_button, pattern="merge_videos"))
    app.add_handler(CallbackQueryHandler(handle_status, pattern="check_status"))
    app.add_handler(CallbackQueryHandler(handle_choice, pattern="compress|split"))
    app.add_handler(CallbackQueryHandler(skip_thumbnail, pattern="skip_thumbnail"))
